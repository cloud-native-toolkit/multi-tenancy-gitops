...
1. Edit the Services layer `${GITOPS_PROFILE}/2-services/kustomization.yaml` by uncommenting the following line to install  File Gateway, commit and push the changes and synchronize the `services` Application in the ArgoCD console:
...
>> This is to avoid going through the job again & creating the database
- Step 1:
    ```bash
    cd multi-tenancy-gitops-services/instances/ibm-sfg-b2bi
    ```
