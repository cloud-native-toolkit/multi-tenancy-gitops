...
1. Generate Helm Chart values.yaml for the Sterling File Gateway Helm Chart in the `multi-tenancy-gitops-services` repo; note that the default storage class is using `managed-nfs-storage` - if you are installing on ODF, set `RWX_STORAGECLASS=ocs-storagecluster-cephfs`.
    ...
    >  💡 **NOTE**  
    > If you want to deploy the custom init container with the S3 SDK resources, go the generated values.yaml and uncomment the section for the S3 custom init container and remove/comment the default init container section.  
...
### Setting Up S3-B2bi Custom Init Container 

A step by step guide on how to setup an Init Container for external resources (S3) including all SKDs, doker file, and a step by step guide, please refer to the resouces in the following URL:

 https://github.com/ShadyIBM/s3-Init-container
